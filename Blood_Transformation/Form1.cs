...
namespace Blood_Transformation
{
    public partial class Form1 : MaterialForm
    {
        ...
        private void login_button_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();  // "mn"is used in database programming to open a connection to a database
                string sql = "SELECT * FROM accounts WHERE Email = '" + txtbox_email.Text + "'";
                SqlCommand command = new SqlCommand(sql, connection);
                SqlDataReader reader = command.ExecuteReader();
                
                // check if password is correct
                if (reader.Read())  // "mn" check pass on DB
                
                    // "mn" it check the password that i enterare the one in the DB or not
                    if (reader["Password"].ToString() == txt_confirm_password.Text)
                    {
                        ...
                        Current_User.user_id = Convert.ToInt32(reader["accounts_ID"].ToString());  // xx
